...
void trainLeptonID(TString name, 
		   TString sig1file, 
		   TString sig2file, 
		   TString bkg1file, 
		   TString bkg2file, 
  ...
  if (name.Contains("nanoAOD_UL")) {
    if (name.Contains("_mu")) {
      factory->AddVariable("Muon_pt",                        'D');
      factory->AddVariable("Muon_eta",                       'D');
      factory->AddVariable("Muon_jetNDauCharged",            'D'); // Muon_jetNDauChargedMVASel
      factory->AddVariable("Muon_miniPFRelIso_chg",          'D'); // Muon_miniRelIsoCharged
      factory->AddVariable("Muon_miniRelIsoNeutral",         'D');
      factory->AddVariable("Muon_jetPtRelv2",                'D');
      // factory->AddVariable("Muon_jetPtRatio := min(LepGood_jetPtRatiov2,1.5)", 'D'); // jetPtRatiov2 = ratio without subtracting the muon itself?
      // factory->AddVariable("Muon_jetBTagCSV := max(Muon_jetBTagCSV,0)",        'D'); // is it possible to get this from nanoAOD?
      factory->AddVariable("Muon_sip3d",                     'D'); 
      factory->AddVariable("Muon_dxy := log(abs(Muon_dxy))", 'D');
      factory->AddVariable("Muon_dz  := log(abs(Muon_dz))",  'D');
      factory->AddVariable("Muon_segmentComp",               'D'); // Muon_segmentCompatibility
      lepton += "Muon_miniPFRelIso_all < 0.4 && Muon_sip3d < 8";
    }
    else if (name.Contains("_el")) {
      factory->AddVariable("Electron_pt",                            'D');
      factory->AddVariable("Electron_eta",                           'D');
      factory->AddVariable("Electron_jetNDauCharged",                'D'); // Electron_jetNDauChargedMVASel
      factory->AddVariable("Electron_miniRelIso_chg",                'D'); // Electron_miniRelIsoCharged
      factory->AddVariable("Electron_miniRelIsoNeutral",             'D');
      factory->AddVariable("Electron_jetPtRelv2",                    'D');
      // factory->AddVariable("Electron_jetPtRatio := min(LepGood_jetPtRatiov2,1.5)", 'D'); // jetPtRatiov2 = ratio without subtracting the muon itself?
      // factory->AddVariable("Electron_jetBTagCSV := max(LepGood_jetBTagCSV,0)",     'D'); // is it possible to get this from nanoAOD? 
      factory->AddVariable("Electron_sip3d",                         'D'); 
      factory->AddVariable("Electron_dxy := log(abs(Electron_dxy))", 'D');
      factory->AddVariable("Electron_dz  := log(abs(Electron_dz))",  'D');
      factory->AddVariable("Electron_mvaFall17V2noIso",              'D'); // Electron_mvaIdSpring16HZZ
  ...
  else if (name.Contains("forMoriond")) {
    ...
    if (name.Contains("_mu")) {
      ...
      if (name.Contains("_eleHZZ")) 
	factory->AddVariable("LepGood_mvaIdSpring16HZZ",'D');
      else if (name.Contains("_eleGP"))
	factory->AddVariable("LepGood_mvaIdSpring16GP",'D');
      else if (name.Contains("_eleOLD")) 
	factory->AddVariable("LepGood_mvaIdSpring15",'D');
      else assert(0);
    }
    else { 
      std::cerr << "ERROR: must either be electron or muon." << std::endl; return; 
    }
  }
  ...
  } // end name.Contains() leptons cases 
  ...
  if (!doMultiClass){
    if (!dSig2) 
      factory->AddSignalTree(dSig1, wSig);
    else {
    ...
    if (!dSig2) 
      factory->AddTree(dSig1,"signal",wSig, "LepGood_mcMatchId!=0");
    else {
  ...
  if (file_for_sigW_1 != "" || file_for_sigW_2 != "")
    factory->SetSignalWeightExpression("addW*xsec*genWeight");
  else
    factory->SetSignalWeightExpression("xsec*genWeight");
  if (file_for_bkgW_1!="" || file_for_bkgW_2!="") 
    factory->SetBackgroundWeightExpression("addW*xsec*genWeight");
